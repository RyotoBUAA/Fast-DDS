...
if [ "$USE_LIBFUZZER" = true ]; then
    LIBFUZZER_BIN="${SCRIPT_DIR}/build_libfuzzer/libfuzzer_harness"
    
    if [ ! -f "$LIBFUZZER_BIN" ]; then
        print_error "libFuzzer harness 未找到，请先运行: ./build_advanced.sh --libfuzzer"
        exit 1
    fi
    ...
    echo ""
    echo "配置:"
    echo "  种子目录: ${SCRIPT_DIR}/seeds/"
    echo "  字典文件: ${SCRIPT_DIR}/dictionaries/rtps.dict"
    echo "  输出目录: ${SCRIPT_DIR}/corpus/"
    echo ""
    
    "$LIBFUZZER_BIN" \
        "${SCRIPT_DIR}/seeds/" \
        "${SCRIPT_DIR}/corpus/" \
        -dict="${SCRIPT_DIR}/dictionaries/rtps.dict" \
        -max_len=65536 \
        -print_final_stats=1 \
...
if [ "$USE_AFL" = true ]; then
    AFL_BIN="${SCRIPT_DIR}/build_afl/afl_harness"
    
    if [ ! -f "$AFL_BIN" ]; then
        print_error "AFL harness 未找到，请先运行: ./build_advanced.sh --afl"
        exit 1
    fi
    ...
    afl-fuzz \
        -i "${SCRIPT_DIR}/seeds/" \
        -o "${SCRIPT_DIR}/output/" \
        -x "${SCRIPT_DIR}/dictionaries/rtps.dict" \
        -- "$AFL_BIN" @@
...
# 检查构建
if [ ! -d "$BUILD_DIR" ]; then
    print_error "构建目录不存在，请先运行: ./build_advanced.sh --basic"
    exit 1
fi
...
if [ ! -f "$FUZZER_BIN" ]; then
    ...
    print_info "请运行: ./build_advanced.sh --basic"
    exit 1
fi
...
if [ "$WITH_MONITOR" = true ]; then
    if [ ! -f "$MONITOR_BIN" ]; then
        ...
        print_info "启动监控节点..."
        "$MONITOR_BIN" > "${SCRIPT_DIR}/monitor_advanced.log" 2>&1 &
        MONITOR_PID=$!
...
# 运行模糊器
$FUZZER_CMD 2>&1 | tee "${SCRIPT_DIR}/advanced_fuzzer.log"
...
# 显示结果摘要
if [ -d "${SCRIPT_DIR}/crashes" ]; then
    CRASH_COUNT=$(ls -1 "${SCRIPT_DIR}/crashes/" 2>/dev/null | wc -l)
    if [ "$CRASH_COUNT" -gt 0 ]; then
        echo -e "${RED}发现 $CRASH_COUNT 个崩溃！${NC}"
        echo "查看: ls ${SCRIPT_DIR}/crashes/"
    fi
fi

if [ -d "${SCRIPT_DIR}/hangs" ]; then
    HANG_COUNT=$(ls -1 "${SCRIPT_DIR}/hangs/" 2>/dev/null | wc -l)
    if [ "$HANG_COUNT" -gt 0 ]; then
        echo -e "${YELLOW}发现 $HANG_COUNT 个挂起！${NC}"
        echo "查看: ls ${SCRIPT_DIR}/hangs/"
    fi
fi

echo ""
echo "日志文件:"
echo "  模糊器日志: ${SCRIPT_DIR}/advanced_fuzzer.log"
if [ "$WITH_MONITOR" = true ]; then
    echo "  监控器日志: ${SCRIPT_DIR}/monitor_advanced.log"
fi
